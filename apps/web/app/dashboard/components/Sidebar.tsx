...
const NavLink = ({ href, icon: Icon, children }) => {
  ...
  return (
    <Link href={href}>
      <a className={`flex items-center gap-3 rounded-lg px-3 py-2 transition-all ${isActive ? 'bg-gray-200 text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}>
      ...
      <div className="flex h-full max-h-screen flex-col gap-2">
        ...
        <div className="flex-1 overflow-auto py-2">
          <nav className="grid items-start px-4 text-sm font-medium">
            {links.map(link => <NavLink key={link.href} {...link}>{link.label}</NavLink>)}
          </nav>
        </div>
      </div>
    </div>
