...
export default function NotificationsPage() {
  ...
  const fetchHistory = async (): Promise<void> => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3000"}/api/notifications/history`, {
        headers: {
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-4 rounded shadow">
            <h2 className="text-xl font-semibold mb-2">Notifikasi Real-time (Live)</h2>
            {notifications.length === 0 ? (
                <p className="text-gray-500">Belum ada notifikasi baru sejak Anda membuka halaman ini.</p>
            ) : (
                     ...
                     <tbody>
                         {history.map((item) => (
                             <tr key={item.id} className="border-b hover:bg-gray-50">
                                 <td className="p-2">{new Date(item.sent_at).toLocaleString()}</td>
                                 <td className="p-2">{item.channel}</td>
                                 <td className="p-2">
                                     <div className="font-medium">{item.subject}</div>
                                     <div className="text-gray-600 truncate max-w-xs">{item.message}</div>
                                 </td>
                                 <td className={`p-2 font-semibold ${item.status === 'SENT' ? 'text-green-600' : 'text-red-600'}`}>
                                     {item.status}
                                 </td>
                             </tr>
                         ))}
                         {history.length === 0 && (
                             <tr>
                                 <td colSpan={4} className="p-4 text-center text-gray-500">Tidak ada riwayat notifikasi.</td>
                             </tr>
                         )}
                     </tbody>
                 </table>
