...
export default function SessionCard({ session }: SessionCardProps) {
    ...
    const sessionActionMutation = useMutation({
        mutationFn: (action: 'cancel' | 'selesaikan') => api.post(`/bimbingan/sesi/${session.id}/${action}`),
        onSuccess: () => {
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg border">
      <div className="flex justify-between items-center">
        <div>
          <p className="font-semibold">{new Date(session.tanggal_bimbingan).toLocaleDateString()}</p>
      ...
      <div className="flex gap-2 mt-4">
        <button onClick={() => sessionActionMutation.mutate('selesaikan')} className="flex items-center gap-1 text-sm bg-green-500 text-white p-2 rounded">
            <CheckCircle size={14} /> Selesaikan
        </button>
        <button onClick={() => sessionActionMutation.mutate('cancel')} className="flex items-center gap-1 text-sm bg-red-500 text-white p-2 rounded">
