...
import { Loader, Info, Send, Star, List, Calendar, Clock, Home } from 'lucide-react';
...
function PenilaianForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 mt-4 pt-4 border-t border-gray-200">
      <h4 className="text-lg font-semibold text-gray-800">Formulir Penilaian Baru</h4>
      {error && <div className="bg-red-100 text-red-700 p-3 rounded-md">{error}</div>}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor={`aspek-${sidangId}`} className="block text-sm font-medium text-gray-700">
            Aspek Penilaian
          </label>
          <input
            id={`aspek-${sidangId}`}
        ...
        <div>
          <label htmlFor={`skor-${sidangId}`} className="block text-sm font-medium text-gray-700">
            Skor (0-100)
          </label>
          <input
            id={`skor-${sidangId}`}
      ...
      <div>
        <label htmlFor={`komentar-${sidangId}`} className="block text-sm font-medium text-gray-700">
          Komentar
        </label>
        <textarea
      ...
      <div className="flex justify-end">
        ...
        >
          {submitting ? <Loader className="animate-spin mr-2" size={16} /> : <Send size={16} className="mr-2" />}
          Submit Nilai
        </button>
      </div>
    </form>
    ...
    <div className="space-y-8">
      ...
      {assignedSidang.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center text-gray-500 bg-white p-12 rounded-2xl shadow-lg">
          ...
          <p className="mt-1">Saat ini tidak ada sidang yang ditugaskan kepada Anda untuk dinilai.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {assignedSidang.map((sidang) => (
            <div key={sidang.id} className="bg-white p-6 rounded-2xl shadow-lg">
              <div className="mb-4">
                <h2 className="text-2xl font-bold text-gray-900">{sidang.tugasAkhir.mahasiswa.user.name}</h2>
                <p className="text-md text-gray-600">{sidang.tugasAkhir.judul}</p>
                {sidang.jadwalSidang[0] && (
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500 mt-2">
                    <span className="flex items-center gap-2"><Calendar size={14} /> {new Date(sidang.jadwalSidang[0].tanggal).toLocaleDateString()}</span>
                    <span className="flex items-center gap-2"><Clock size={14} /> {sidang.jadwalSidang[0].waktu_mulai}</span>
                    <span className="flex items-center gap-2"><Home size={14} /> {sidang.jadwalSidang[0].ruangan.nama_ruangan}</span>
                  </div>
                )}
              </div>

              <div className="bg-gray-50 p-4 rounded-lg">
                <h4 className="text-md font-semibold text-gray-800 mb-2 flex items-center"><List size={18} className="mr-2"/> Skor yang Telah Diberikan</h4>
                {sidang.nilaiSidang.length > 0 ? (
                  <ul className="space-y-2">
                    {sidang.nilaiSidang.map((n) => (
                      <li key={n.id} className="flex justify-between items-center bg-white p-2 rounded-md shadow-sm">
                        <span className="text-sm font-medium text-gray-700">{n.aspek}</span>
                        <span className="text-sm font-bold text-maroon-800 bg-maroon-100 px-2 py-1 rounded-full">{n.skor}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-sm text-gray-500">Belum ada skor yang dimasukkan.</p>
                )}
              </div>
